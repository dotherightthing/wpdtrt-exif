...
if ( !function_exists( 'wpdtrt_attachment_gps_menu' ) ) {
   ...
   */
  function wpdtrt_attachment_gps_menu() {
    ...
    add_options_page(
      ...
      'manage_options',
      'wpdtrt-attachment-gps',
      'wpdtrt_attachment_gps_options_page'
    );
  }

  add_action('admin_menu', 'wpdtrt_attachment_gps_menu');
 ...
 */
if ( !function_exists( 'wpdtrt_attachment_gps_options_page' ) ) {

  /**
   * Render the appropriate UI on Settings > DTRT Attachment GPS
   *
   ...
   *       a. The user's selection: 'wpdtrt_attachment_gps_datatype',
   *          so that the dataset can be regenerated after a specified amount of time
   *       b. The data set generated by the API: 'wpdtrt_attachment_gps_data'
   ...
   * @since       0.1.0
   */
  function wpdtrt_attachment_gps_options_page() {
     ...
     */
    global $wpdtrt_attachment_gps_options;

    if ( isset( $_POST['wpdtrt_attachment_gps_form_submitted'] ) ) {

      // check that the form submission was legitimate
      $hidden_field = esc_html( $_POST['wpdtrt_attachment_gps_form_submitted'] );

      if ( $hidden_field === 'Y' ) {

        // 1. get user preferences from form submission
        $wpdtrt_attachment_gps_datatype = esc_html( $_POST['wpdtrt_attachment_gps_datatype'] );

        // 2. send user preferences to API to get results
        $wpdtrt_attachment_gps_data = wpdtrt_attachment_gps_get_data(
          $wpdtrt_attachment_gps_datatype
        );

        // 3a. store user preferences in options object
        $wpdtrt_attachment_gps_options['wpdtrt_attachment_gps_datatype'] = $wpdtrt_attachment_gps_datatype;

        // 3b. store API results in options object
        $wpdtrt_attachment_gps_options['wpdtrt_attachment_gps_data'] = $wpdtrt_attachment_gps_data;

        // 3c. store timestamp in options object
        $wpdtrt_attachment_gps_options['last_updated'] = time(); // UNIX timestamp for the current time
         ...
         */
        update_option( 'wpdtrt_attachment_gps', $wpdtrt_attachment_gps_options, null );
      }
    }
     ...
     */
    require_once(WPDTRT_ATTACHMENT_GPS_PATH . 'templates/wpdtrt-attachment-gps-options.php');
  }
