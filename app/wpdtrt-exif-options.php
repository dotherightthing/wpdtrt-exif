...
if ( !function_exists( 'wpdtrt_exif_menu' ) ) {
  ...
  function wpdtrt_exif_menu() {
    ...
    add_options_page(
      ...
      'manage_options',
      'wpdtrt-exif',
      'wpdtrt_exif_options_page'
    );
  }

  add_action('admin_menu', 'wpdtrt_exif_menu');
 ...
 */
if ( !function_exists( 'wpdtrt_exif_options_page' ) ) {

  /**
   * Render the appropriate UI on Settings > DTRT EXIF
   *
   ...
   *       a. The user's selection: 'wpdtrt_exif_datatype',
   *          so that the dataset can be regenerated after a specified amount of time
   *       b. The data set generated by the API: 'wpdtrt_exif_data'
   ...
   * @since       0.1.0
   */
  function wpdtrt_exif_options_page() {
     ...
     */
    global $wpdtrt_exif_options;

    if ( isset( $_POST['wpdtrt_exif_form_submitted'] ) ) {

      // check that the form submission was legitimate
      $hidden_field = esc_html( $_POST['wpdtrt_exif_form_submitted'] );

      if ( $hidden_field === 'Y' ) {

        // 1. get user preferences from form submission
        $wpdtrt_exif_datatype = esc_html( $_POST['wpdtrt_exif_datatype'] );

        // 2. send user preferences to API to get results
        $wpdtrt_exif_data = wpdtrt_exif_get_data(
          $wpdtrt_exif_datatype
        );

        // 3a. store user preferences in options object
        $wpdtrt_exif_options['wpdtrt_exif_datatype'] = $wpdtrt_exif_datatype;

        // 3b. store API results in options object
        $wpdtrt_exif_options['wpdtrt_exif_data'] = $wpdtrt_exif_data;

        // 3c. store timestamp in options object
        $wpdtrt_exif_options['last_updated'] = time(); // UNIX timestamp for the current time
         ...
         */
        update_option( 'wpdtrt_exif', $wpdtrt_exif_options, null );
      }
    }
     ...
     */
    require_once(WPDTRT_EXIF_PATH . 'templates/wpdtrt-exif-options.php');
  }
